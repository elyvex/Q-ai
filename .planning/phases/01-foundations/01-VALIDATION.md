...
The plans define fourteen implementation tasks across five waves. No task is complete merely because it appears in this file. The first executable task, `01-01-01`, must refresh the live dirty/untracked path set and run `scripts/verify-phase1-preservation.sh capture` before creating TASK-001 or touching production code. Every task records a path-level preflight, allowed-hunk manifest, and preservation check; final preservation is checked before the record checker. The active-to-completed task-file/index handoff is owned exclusively by `01-05-02`; `01-05-03` records post-move consistency and may rerun gates but cannot move the task or change either index.
...
| **Final gate** | `cargo fmt --all -- --check && cargo clippy --workspace --all-targets -- -D warnings && cargo test --workspace && cargo run -q -p xtask -- arch-check && cargo run -q -p xtask -- migrate-check && cargo run -p xtask -- ci && sh scripts/verify-phase1-preservation.sh check --all && sh scripts/verify-phase1-records.sh` |
...
- **After every task commit:** run that task's automated command, then run the task's declared `scripts/verify-phase1-preservation.sh check --task <id>` and `cargo fmt --all -- --check` and `cargo clippy --workspace --all-targets -- -D warnings` when Rust files changed.
...
- **Before phase verification:** run the final gate, record the real exit outcomes, run `sh scripts/verify-phase1-preservation.sh check --all`, and perform the independent cross-record check below.
...
- **Real job-control evidence:** task `01-03-03` runs `crates/cli/tests/jobs.rs` against `CARGO_BIN_EXE_qai` and covers retry, cancel, inspection, redaction, centralized success/error exits, and JSON output; storage-only or application-only evidence is insufficient.
- **Real host evidence:** task `01-04-01` runs the real `qai serve` child path; task `01-04-02` runs its named real-binary enqueue-only filter without the trycmd snapshots; task `01-04-03` runs the complete host-backed real-binary Quran target after synchronization. An application-only worker test is insufficient.
...
|---|---|---:|---|---|---|---|---|---|---|---|
| 01-01-01 | 01 | 1 | REQ-product-vision, REQ-engineering-baseline, REQ-cli-api, REQ-architecture-principles-quality | D-01, D-02, D-03, D-04, D-05, D-06, D-08, D-12 | T-01-CFG, T-01-DOCTOR, T-01-MIG, T-01-AUDIT | Real `CARGO_BIN_EXE_qai` fresh-workspace path shows seven groups, origin-bearing config, no doctor-created path, explicit `qai db migrate`, and an empty-chain audit result; the dynamic preservation capture/check covers every dirty tracked and untracked path before and after the tracer. | CLI/E2E + preservation | `cargo test -p cli --test foundation -- --nocapture && sh scripts/verify-phase1-preservation.sh check --task 01-01-01` | `crates/cli/tests/foundation.rs` (new), `scripts/verify-phase1-preservation.sh` (new), active TASK-001/index | pending |
| 01-01-02 | 01 | 1 | REQ-product-principles, REQ-engineering-baseline, REQ-cli-api | D-01, D-02, D-03, D-07, D-08 | T-01-CFG | Effective file/env/CLI/default precedence, dotted origins, keyed lookup, secret redaction, and identical human/JSON code + remedy + next command for every non-passing validation result. | config/CLI | `cargo test -p testkit --test config_precedence && cargo test -p cli --test foundation -- --nocapture` | `crates/testkit/tests/config_precedence.rs`, `crates/cli/tests/foundation.rs`, active TASK-001 | pending |
| 01-01-03 | 01 | 1 | REQ-product-principles, REQ-engineering-baseline, REQ-storage-architecture, REQ-cli-api | D-01, D-05, D-06, D-12 | T-01-DOCTOR, T-01-MIG, T-01-AUDIT, T-01-SQL | Fresh, missing, pending, checksum-mismatch, current, restricted, and tampered-audit cases prove metadata-only doctor and no ordinary creating open; real binary `qai audit verify` human and JSON cases for both tampered sequences and gaps assert the stable code/remedy/next command and centralized `exit_code::VALIDATION`. | CLI/integration | `cargo test -p cli --test foundation -- --nocapture && cargo test -p application --lib db` | `crates/audit/src/lib.rs`, `crates/cli/src/lib.rs`, `crates/cli/src/doctor.rs`, `crates/application/src/quran_cli.rs`, `crates/cli/tests/foundation.rs`, active TASK-001 | pending |
| 01-02-01 | 02 | 2 | REQ-product-principles, REQ-engineering-baseline, REQ-storage-architecture, REQ-cli-api | D-01, D-02, D-03, D-09, D-10 | T-02-ATOMICITY, T-02-PROVENANCE, T-02-CANONICAL | One application `AuditedMutation` path commits domain, provenance/outbox, and chained audit records together; an injected failure leaves no partial row and importer staging cannot activate canonical data, with exact D-09/D-10 evidence appended to the active TASK-001 record. | real SQLite/integration | `cargo test -p application --test phase1_foundation audited_mutation -- --nocapture && cargo test -p application --lib -- --nocapture` | `crates/application/tests/phase1_foundation.rs` (new), audited lifecycle seams, active TASK-001 | pending |
| 01-02-02 | 02 | 2 | REQ-product-principles, REQ-engineering-baseline, REQ-storage-architecture, REQ-cli-api | D-01, D-03, D-09, D-10, D-12 | T-02-ATOMICITY, T-02-RECOVERY, T-02-SQL | Independent real-SQLite fault injection covers every required write boundary, contiguous chain, tamper sequence, and a non-mutating persisted `qai audit verify` remedy; the task appends its test evidence to the active TASK-001 record. | real SQLite/integration | `cargo test -p application --test phase1_foundation && cargo test -p storage-sqlite --test commit_bounds_outbox --test integrity_audit` | `crates/application/tests/phase1_foundation.rs`, `commit_bounds_outbox.rs`, `integrity_audit.rs`, `docs/04-tasks/active/TASK-001-foundation-gap-closure.md` | pending |
| 01-03-01 | 03 | 3 | REQ-engineering-baseline, REQ-storage-architecture, REQ-cli-api | D-01, D-02, D-03, D-14, D-15 | T-03-LEASE, T-03-RETRY, T-03-CHECKPOINT | A task-local jobs contract test plus real-SQLite adapter cases prove immediate named-checkpoint visibility and recovery, per-kind policy isolation, eligible/ineligible explicit retry, request-only cancellation, and non-owner heartbeat/checkpoint/finish rejection; D-14/D-15 evidence is appended to the active TASK-001 record. | jobs contract/real SQLite | `cargo test -p jobs --lib && cargo test -p storage-sqlite --test recovery_jobs -- --nocapture` | `crates/jobs/src/lib.rs` inline test, existing `crates/storage-sqlite/tests/recovery_jobs.rs` target, active TASK-001 | pending |
| 01-03-02 | 03 | 3 | REQ-product-principles, REQ-engineering-baseline, REQ-storage-architecture, REQ-cli-api | D-01, D-03, D-11, D-14, D-15, D-16 | T-03-LEASE, T-03-RETRY, T-03-CHECKPOINT, T-03-CONCURRENCY | Real SQLite crash/resume, per-kind exhaustion, cancellation before/during/after boundary, and ownership cases run in this task; the three truthful cancellation dispositions are asserted and D-11/D-14/D-15/D-16 evidence is appended to the active TASK-001 record. | worker/real SQLite | `cargo test -p jobs --lib -- --nocapture && cargo test -p storage-sqlite --test recovery_jobs -- --nocapture` | `crates/jobs/src/worker.rs`, `crates/storage-sqlite/tests/recovery_jobs.rs`, active TASK-001 | pending |
| 01-03-03 | 03 | 3 | REQ-product-principles, REQ-engineering-baseline, REQ-storage-architecture, REQ-cli-api | D-01, D-03, D-11, D-14, D-15, D-16 | T-03-LEASE, T-03-SECRET, T-03-CONCURRENCY | Real-SQLite lifecycle audit plus a real `qai job` binary target prove retry, cancel, inspection, redaction, happy/error exit codes, and JSON output; no CLI behavior is deferred, and the task appends D-11/D-14/D-15/D-16 evidence to the active TASK-001 record. | application/CLI/real SQLite | `cargo test -p application --lib job_queue && cargo test -p storage-sqlite --test phase1_jobs_audit --test recovery_jobs && cargo test -p cli --test jobs -- --nocapture` | `crates/storage-sqlite/tests/phase1_jobs_audit.rs` (new), `crates/cli/tests/jobs.rs` (new), job CLI seam, `docs/04-tasks/active/TASK-001-foundation-gap-closure.md` | pending |
| 01-04-01 | 04 | 4 | REQ-product-vision, REQ-product-principles, REQ-goals-non-goals, REQ-engineering-baseline, REQ-storage-architecture, REQ-cli-api, REQ-architecture-principles-quality | D-01, D-02, D-03, D-04, D-05, D-13, D-14, D-16 | T-04-HOST, T-04-SHUTDOWN, T-04-LEASE | A real `qai serve` child on an isolated loopback port claims a queued job, proves readiness, receives shutdown, awaits a durable terminal/checkpoint state, and exits without a detached worker; missing/pending DB reports the migration remedy, and host evidence is appended to the active TASK-001 record. | application + real CLI host | `cargo test -p application --test phase1_jobs_host -- --nocapture && cargo test -p cli --test quran -- --nocapture` | `crates/application/tests/phase1_jobs_host.rs` (new), `crates/cli/tests/quran.rs`, active TASK-001 | pending |
| 01-04-02 | 04 | 4 | REQ-product-vision, REQ-product-principles, REQ-goals-non-goals, REQ-engineering-baseline, REQ-storage-architecture, REQ-cli-api, REQ-architecture-principles-quality | D-01, D-04, D-05, D-09, D-10, D-11, D-13, D-14, D-15, D-16 | T-04-ENQUEUE, T-04-LEASE, T-04-SECRET | The one-shot import returns a durable queued id/state before host processing; the named real-binary `enqueue_only_import_is_queued_without_host` filter proves that response without executing or updating the trycmd snapshots owned by 01-04-03; the application test proves explicit later host processing and no inline worker or canonical activation. | application + named real CLI filter | `cargo test -p application --test quran_import -- --nocapture && cargo test -p cli --test quran enqueue_only_import_is_queued_without_host -- --nocapture` | `crates/application/tests/quran_import.rs`, `crates/cli/tests/quran.rs` (named test/filter), queued result seam, active TASK-001 | pending |
| 01-04-03 | 04 | 4 | REQ-product-vision, REQ-product-principles, REQ-engineering-baseline, REQ-cli-api | D-03, D-04, D-13 | T-04-HOST, T-04-ENQUEUE, T-04-SCOPE | The existing four trycmd suites run against a real `qai serve` child, synchronize persisted terminal state before dependent commands, clean up child/port/data paths, retain unrelated Quran/search behavior, and append the snapshot evidence to the active TASK-001 record; the enqueue-only named test remains owned by 01-04-02. | real CLI/trycmd | `cargo test -p cli --test quran -- --nocapture` | four existing Quran snapshots, `crates/cli/tests/quran.rs` host harness, active TASK-001 | pending |
| 01-05-01 | 05 | 5 | REQ-architecture-principles-quality, REQ-engineering-baseline | D-01, D-02, D-03, D-04 | T-05-ARCH, T-05-EXTERNAL, T-05-SUPPLYCHAIN | Inline `xtask/src/arch.rs` tests prove allowed/forbidden registry and git edges plus path fail-closed behavior; current metadata-derived allowances remain green, no package is installed, and the evidence is appended to the active TASK-001 record. | build policy | `cargo test -p xtask && cargo run -q -p xtask -- arch-check` | `xtask/src/arch.rs`, `xtask/allowlist.toml`, active TASK-001 | pending |
| 01-05-02 | 05 | 5 | REQ-product-vision, REQ-product-principles, REQ-goals-non-goals, REQ-engineering-baseline, REQ-storage-architecture, REQ-cli-api, REQ-architecture-principles-quality | D-01, D-02, D-03 | T-05-STATUS, T-05-SCOPE | After a captured green pre-move final gate, the sole owner creates and syntax-checks the record checker, atomically performs the task-file move, removes the TASK-001 active-index entry, and adds exactly one completed-index entry; the preservation check proves unrelated index bytes and every live path remain unchanged outside declared entries. | repository records + preservation | `cargo fmt --all -- --check && cargo clippy --workspace --all-targets -- -D warnings && cargo test --workspace && cargo run -q -p xtask -- arch-check && cargo run -q -p xtask -- migrate-check && cargo run -p xtask -- ci && sh -n scripts/verify-phase1-records.sh && sh scripts/verify-phase1-preservation.sh check --task 01-05-02 && test -f docs/04-tasks/completed/TASK-001-foundation-gap-closure.md && test ! -e docs/04-tasks/active/TASK-001-foundation-gap-closure.md && test "$(grep -F -c 'TASK-001-foundation-gap-closure' docs/04-tasks/active/README.md)" -eq 0 && test "$(grep -F -c 'TASK-001-foundation-gap-closure' docs/04-tasks/completed/README.md)" -eq 1 && grep -Fq 'Status: Completed' docs/04-tasks/completed/TASK-001-foundation-gap-closure.md && git diff --check -- docs/04-tasks` | active/completed TASK-001, both indexes, pre-move gate evidence, record checker | pending |
| 01-05-03 | 05 | 5 | REQ-product-vision, REQ-product-principles, REQ-goals-non-goals, REQ-engineering-baseline, REQ-storage-architecture, REQ-cli-api, REQ-architecture-principles-quality | D-01, D-02, D-03, D-04, D-05, D-06, D-07, D-08, D-09, D-10, D-11, D-12, D-13, D-14, D-15, D-16 | T-01-CFG, T-01-DOCTOR, T-01-MIG, T-01-AUDIT, T-01-SQL, T-02-ATOMICITY, T-02-PROVENANCE, T-02-CANONICAL, T-02-RECOVERY, T-02-SQL, T-03-LEASE, T-03-RETRY, T-03-CHECKPOINT, T-03-SECRET, T-03-CONCURRENCY, T-04-HOST, T-04-SHUTDOWN, T-04-ENQUEUE, T-04-LEASE, T-04-SECRET, T-04-SCOPE, T-05-ARCH, T-05-EXTERNAL, T-05-SUPPLYCHAIN, T-05-STATUS, T-05-SCOPE, T-05-INFO | Rerun the final gate after the completed TASK-001 task/index handoff, record actual outcomes, update validation/task-done rollup/status/changelog with additive-only hunks, and run the pre-existing record checker after a dynamic per-path preservation check. | final cross-record gate | `cargo fmt --all -- --check && cargo clippy --workspace --all-targets -- -D warnings && cargo test --workspace && cargo run -q -p xtask -- arch-check && cargo run -q -p xtask -- migrate-check && cargo run -p xtask -- ci && sh scripts/verify-phase1-preservation.sh check --all && sh scripts/verify-phase1-records.sh && sh scripts/verify-phase1-preservation.sh check --all && git diff --check -- scripts/verify-phase1-records.sh scripts/verify-phase1-preservation.sh .planning/phases/01-foundations/01-VALIDATION.md docs/04-tasks docs/06-progress/task-done-rollup.md docs/06-progress/status.md CHANGELOG.md` | validation, task-done rollup, status, changelog, completed record, existing record checker/preservation companion | pending |
...
|---|---|---|
| Seven-group real-binary help and fresh-workspace operator path | 01-01-01 | `crates/cli/tests/foundation.rs` creates and runs the real `qai` test; `scripts/verify-phase1-preservation.sh capture` runs first and `check --task 01-01-01` runs last. |
...
| Metadata-only doctor, readiness states, and public tampered/gap audit output | 01-01-03 | Its own fresh/pending/current/restricted/tampered/gap cases in both human and JSON real-binary output. |
| Same-UnitOfWork first proof | 01-02-01 | One commit and one rollback at the new audited composition boundary. |
| Failure matrix and recovery guidance | 01-02-02 | Independent real-SQLite fault injection, not only the first proof, plus TASK-001 evidence append. |
| Named checkpoint, retry, cancellation request, and ownership contracts | 01-03-01 | Task-local jobs contract test and real-SQLite adapter cases run before worker expansion. |
| Crash, retry, cancellation, and ownership behavior | 01-03-02 | Task-local real-SQLite cases. |
| Lifecycle audit and operator controls | 01-03-03 | Task-local real-SQLite lifecycle-audit case and real `qai job` binary target for retry, cancel, inspection, redaction, exit codes, and output, followed by an explicit TASK-001 evidence append. |
| Real `qai serve` ownership and joined shutdown | 01-04-01 | Real child process plus application host test. |
| Enqueue-only import and later host-owned terminal state | 01-04-02 | Application test plus the named real-binary `enqueue_only_import_is_queued_without_host` filter; no trycmd snapshot dependency. |
| Host-backed corpus snapshot boundary | 01-04-03 | Existing `quran` target runs all four trycmd suites after the host synchronization changes. |
| Registry/git policy mutation | 01-05-01 | Inline `xtask/src/arch.rs` tests and `arch-check`. |
| TASK-001 closure and final evidence | 01-05-02, 01-05-03 | Pre-move gate and checker creation, sole task/index handoff, then preservation plus record-specific post-move gate. |
...
| REQ-cli-api | Executable `qai`, stable groups, JSON/remedies, queued jobs, cancellable `qai serve`. | 01-01, 01-03, 01-04, 01-05 | Real binary foundation and `qai job` controls, host/import flows, catalog/help tests, and final gate. |
...
| C4 | Background jobs enqueue, lease, checkpoint, and cancel without an external broker. | 01-03-01 through 01-04-03; 01-05-03 | `cargo test -p jobs --lib && cargo test -p storage-sqlite --test recovery_jobs --test phase1_jobs_audit && cargo test -p cli --test jobs -- --nocapture && cargo test -p application --test phase1_jobs_host && cargo test -p cli --test quran -- --nocapture` |
...
| D-12 | `qai audit verify` is the authority for ordering, hashes, continuity, and recovery guidance. | 01-01-03, 01-02-02, 01-05-03 | Real human/JSON tampered and gap cases assert stable code/remedy/next command and centralized exit, shared doctor result, and persisted verifier evidence. |
...
| T-05-STATUS | 01-05-02, 01-05-03 | Repudiation | high | mitigate | Capture gate evidence before the sole task/index handoff and cross-check all task/progress records. |
...
| No unrelated scope edit | Dynamically discovered EXCLUDE paths remain byte-for-byte untouched; shared CHANGELOG/rollup/status/task/index records contain only their declared Phase 1 additive entries. | 01-01-01, 01-05-02, 01-05-03. |

## TASK-001 Lifecycle and Cross-Record Contract

| Order | Owner | Required state/action | Automated check |
|---:|---|---|---|
| 1 | 01-01-01 | Refresh the live dirty/untracked set, run `scripts/verify-phase1-preservation.sh capture`, then create the active TASK-001 record and index entry before production edits; status is `Active`. | `sh scripts/verify-phase1-preservation.sh capture && test -f docs/04-tasks/active/TASK-001-foundation-gap-closure.md` |
| 2 | 01-01 through 01-04 | Run a path-level preservation preflight/check for every task and append task evidence while the task remains active; do not move or mark completed. | `sh scripts/verify-phase1-preservation.sh check --task <task-id> && rg -n 'TASK-001-foundation-gap-closure|D-01|D-16' docs/04-tasks/active/TASK-001-foundation-gap-closure.md` |
...
| 4 | 01-05-02 | Run and capture the pre-move final gate; create and syntax-check the record checker; only after green, atomically move active TASK-001 to completed, remove its active-index entry, and add one completed-index entry while leaving all unrelated index bytes unchanged. Leave the task-done rollup untouched. | `sh -n scripts/verify-phase1-records.sh && sh scripts/verify-phase1-preservation.sh check --task 01-05-02 && test -f docs/04-tasks/completed/TASK-001-foundation-gap-closure.md && test ! -e docs/04-tasks/active/TASK-001-foundation-gap-closure.md && test "$(grep -F -c 'TASK-001-foundation-gap-closure' docs/04-tasks/active/README.md)" -eq 0 && test "$(grep -F -c 'TASK-001-foundation-gap-closure' docs/04-tasks/completed/README.md)" -eq 1 && grep -Fq 'Status: Completed' docs/04-tasks/completed/TASK-001-foundation-gap-closure.md` |
| 5 | 01-05-03 | Rerun/record the final gate; run the preservation verifier and the pre-existing record-specific checker; update validation, task-done rollup, status, and changelog with additive-only hunks. Do not move TASK-001 or change either index again. | `sh scripts/verify-phase1-preservation.sh check --all && sh scripts/verify-phase1-records.sh && sh scripts/verify-phase1-preservation.sh check --all` |

The completed record must contain, at minimum, the following exact groups: all five plan IDs `01-01` through `01-05`; all seven `REQ-*` IDs; all five `C1`–`C5` criterion IDs; all `D-01` through `D-16`; all ten `PROBE-01` through `PROBE-10` dispositions; all threat IDs in the threat register; the final command outcomes; and one statement that owner-gated/deferred items remain outside Phase 1. Task 01-05-02 creates `scripts/verify-phase1-records.sh`; task 01-05-03 runs it after `scripts/verify-phase1-preservation.sh check --all`. The record checker asserts these groups independently per record: validation carries every task/requirement/criterion/decision/probe/threat token and both completion flags; the completed task carries its ID, `Status: Completed`, all plan/requirement/criterion/decision/probe/threat tokens; the newest Phase 1 rollup entry carries TASK-001, all plan/criterion/decision tokens, and final-gate commands; the dated status section carries TASK-001, `Status: Completed`, and all plan/criterion/decision tokens; the Unreleased entry carries TASK-001, all plan/criterion/decision tokens, and the brownfield/non-expansion boundary. A token in one record never substitutes for a token required in another. The companion preservation verifier independently proves every dynamically discovered EXCLUDE byte/diff, MERGE hunk, and APPEND boundary, including the intentionally additive shared records. The active path/index entry must be absent, the completed path/index entry present exactly once, and the status/rollup/changelog must describe the same completion event. No unrelated worktree path may be staged or edited.
...
- [x] Real `qai job` retry/cancel/inspection/redaction/exit/output evidence is assigned to task 01-03-03.
- [x] Real `qai serve` evidence is assigned to the host task, and the enqueue-only filter plus host-backed corpus target are assigned to the import and snapshot tasks respectively.
- [x] `xtask` architecture tests are inline in `xtask/src/arch.rs`.
- [x] TASK-001 ownership, dynamic preservation capture, pre-move gate, record-checker creation, sole task/index handoff, and record-specific post-move consistency checks are explicit.
- [x] D-12 public human/JSON tampered/gap output, centralized exit, and stable remedy/next command are assigned to 01-01-03.
- [x] 01-04-02 has a local named enqueue-only filter independent of the trycmd snapshot rewrites owned by 01-04-03.
- [ ] Actual task rows are green after execution.
- [ ] `wave_0_complete: true` and `nyquist_compliant: true` are set only after the final gate, `sh scripts/verify-phase1-preservation.sh check --all`, and `sh scripts/verify-phase1-records.sh` pass.
