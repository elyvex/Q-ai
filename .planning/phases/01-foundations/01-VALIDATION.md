...
The plans define fourteen implementation tasks across five waves. No task is complete merely because it appears in this file. The executor must create the active `TASK-001-foundation-gap-closure` record before the first production edit, run the task-local command in the owning task, and only then append evidence. The active-to-completed task-file/index handoff is owned exclusively by `01-05-02`; `01-05-03` records post-move consistency and may rerun gates but cannot move the task or change either index.
...
| 01-03-03 | 03 | 3 | REQ-product-principles, REQ-engineering-baseline, REQ-storage-architecture, REQ-cli-api | D-01, D-03, D-11, D-14, D-15, D-16 | T-03-LEASE, T-03-SECRET, T-03-CONCURRENCY | Real-SQLite lifecycle audit plus a real `qai job` binary target prove retry, cancel, inspection, redaction, happy/error exit codes, and JSON output; no CLI behavior is deferred. | application/CLI/real SQLite | `cargo test -p application --lib job_queue && cargo test -p storage-sqlite --test phase1_jobs_audit --test recovery_jobs && cargo test -p cli --test jobs -- --nocapture` | `crates/storage-sqlite/tests/phase1_jobs_audit.rs` (new), `crates/cli/tests/jobs.rs` (new), `crates/cli/Cargo.toml` existing dev-dependency wiring, job CLI seam | pending |
...
| Lifecycle audit and operator controls | 01-03-03 | Task-local real-SQLite lifecycle-audit case and real `qai job` binary target for retry, cancel, inspection, redaction, exit codes, and output. |
...
| TASK-001 closure and final evidence | 01-05-02, 01-05-03 | Pre-move gate, sole task/index handoff, then post-move record-specific gate. |
...
| 4 | 01-05-02 | Run and capture the pre-move final gate; only after green, atomically move active TASK-001 to completed, remove its active-index entry, and add one completed-index entry while leaving all unrelated index bytes unchanged. Leave the task-done rollup untouched. | `test -f docs/04-tasks/completed/TASK-001-foundation-gap-closure.md && test ! -e docs/04-tasks/active/TASK-001-foundation-gap-closure.md && test "$(grep -F -c 'TASK-001-foundation-gap-closure' docs/04-tasks/active/README.md)" -eq 0 && test "$(grep -F -c 'TASK-001-foundation-gap-closure' docs/04-tasks/completed/README.md)" -eq 1 && grep -Fq 'Status: Completed' docs/04-tasks/completed/TASK-001-foundation-gap-closure.md` |
| 5 | 01-05-03 | Rerun/record the final gate; create and run the record-specific checker; update validation, task-done rollup, status, and changelog. Do not move TASK-001 or change either index again. | `sh scripts/verify-phase1-records.sh` |

The completed record must contain, at minimum, the following exact groups: all five plan IDs `01-01` through `01-05`; all seven `REQ-*` IDs; all five `C1`–`C5` criterion IDs; all `D-01` through `D-16`; all ten `PROBE-01` through `PROBE-10` dispositions; all threat IDs in the threat register; the final command outcomes; and one statement that owner-gated/deferred items remain outside Phase 1. Task 01-05-03 creates `scripts/verify-phase1-records.sh` to assert these groups independently per record: validation carries every task/requirement/criterion/decision/probe/threat token and both completion flags; the completed task carries its ID, `Status: Completed`, all plan/requirement/criterion/decision/probe/threat tokens; the newest Phase 1 rollup entry carries TASK-001, all plan/criterion/decision tokens, and final-gate commands; the dated status section carries TASK-001, `Status: Completed`, and all plan/criterion/decision tokens; the Unreleased entry carries TASK-001, all plan/criterion/decision tokens, and the brownfield/non-expansion boundary. A token in one record never substitutes for a token required in another. The active path/index entry must be absent, the completed path/index entry present exactly once, and the status/rollup/changelog must describe the same completion event. No unrelated worktree path may be staged or edited.
...
- [x] TASK-001 ownership, pre-move gate, sole task/index handoff, and record-specific post-move consistency checks are explicit.
- [ ] Actual task rows are green after execution.
- [ ] `wave_0_complete: true` and `nyquist_compliant: true` are set only after the final gate and `sh scripts/verify-phase1-records.sh` pass.
