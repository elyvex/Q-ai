...
**Status: RESOLVED — all five research questions have explicit dispositions before execution.** Each resolution is grounded in the locked Phase 1 boundary or D-01 through D-16; none invents a user preference. `RETAINED ASSUMPTION` identifies an implementation boundary that is now recorded and mapped to executable work, not an unresolved question.

| Research question | Resolution | Disposition | Plan / decision / retained-assumption mapping |
|---|---|---|---|
| RQ-01 — Does Phase 1 require a fully usable generic `source import`, or only the stable foundation source surface plus audited existing lifecycle paths? | **RESOLVED:** Keep the existing `source` group and close the demonstrated Quran source/version staging, approval, activation, rollback, provenance, audit, and outbox seams. Do not add a generic internet/source-catalog workflow or a canonical activation path; those remain with the later source-catalog phase. | RETAINED ASSUMPTION RA-01, explicitly bounded by D-04 and D-09; command presence under D-08 does not create a new mutation surface. | 01-02-01/02, 01-04-02, and the FND-04/FND-06 source-audit rows; D-04, D-08, D-09. |
| RQ-02 — Should `secret set` / `secret delete` gain durable mutation behavior in Phase 1? | **RESOLVED:** No durable secret-value mutation is added. Preserve the existing explicit refusal and reference-only validation/inspection; D-07 governs redacted inspection and D-08 governs the stable group. | RETAINED ASSUMPTION RA-02, not an unresolved product question; a future secret-value workflow needs its own backend, threat model, and audit contract. | 01-01-02 and the 01-05-03 scope gate; D-07, D-08, D-02. |
| RQ-03 — What exact per-kind retry policy should be persisted? | **RESOLVED:** Use a typed per-kind policy in `HandlerRegistry`, snapshot the selected attempt/backoff bounds through existing `JobRecord` fields, and choose exact deterministic constants under D-15’s explicit discretion. Exhaustion and explicit retry remain inspectable. | RESOLVED BY D-15 DISCRETION; A2 is limited to exact constants and serialized names. | 01-03-01/02/03; D-15 and the retained implementation assumption A2. |
| RQ-04 — Should Phase 1 add a schema migration? | **RESOLVED:** No migration is planned. Existing jobs, progress, checkpoint, error, cancellation, audit, provenance, and outbox storage must represent the required state. If implementation proves a durable contract cannot fit, halt for an explicit plan revision/ADR instead of silently adding a migration or weakening locked behavior. | RETAINED ASSUMPTION RA-03, bounded by D-05; an unplanned migration is a stop condition, not a discretionary simplification. | 01-01-03, 01-03-01/02, and 01-05-02 `migrate-check`; D-05 and D-02. |
| RQ-05 — What should `qai serve` do when the database is unmigrated? | **RESOLVED:** Fail before binding the server or starting the worker, emit the centralized migration-required diagnostic with the exact `qai db migrate` remedy and next command, and use the ordinary-command exit mapping. Do not auto-migrate or enter a degraded write-capable mode. | RESOLVED BY D-05; no owner preference is added. | 01-01 readiness, 01-04-01, and the real CLI host test; D-05, D-13, D-03. |

The five rows above are the complete RQ-01 through RQ-05 set. RA-01, RA-02, and RA-03 are visible planning assumptions tied to tests and decision predicates; they must not be silently converted into additional product scope. Any change to one of these dispositions requires a planning revision.
