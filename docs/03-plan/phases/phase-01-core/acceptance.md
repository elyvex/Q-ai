...
| **AC-P1-16** | CLI follows the Phase-0 conventions (verbs after nouns; `--json` on every read command; destructive commands confirm unless `--yes`; documented exit-code table); snapshots cover `qai quran get/context/surah/division/resolve/edition/import/validate/activate/diff/rollback` incl. RTL/Arabic terminal output sanity | CLI snapshot suite (trycmd) | D1.10, D1.13 | `cli/tests/quran/read_flow.trycmd` covers every listed verb (+ `hashes`, `translation`, `--json`, error exits 5/6) with RTL text asserted; the ritual walkthrough is pending | ◐ |
| **AC-P1-17** | `qai doctor --quran` runs all 19 checks (each emitting `Pass/Warn/Fail/Skipped` + remedy + next command), opens the DB **read-only**, `--deep` completes full-corpus hash + token round-trip in **< 30 s** for a standard edition, and `--json` validates against `docs/schemas/doctor.v1.schema.json` | Read-only + schema + timing test | D1.11 | 19 checks + read-only open live-verified; merged single JSON doc passes `cargo xtask validate` against the schema (`cli/tests/doctor_json.rs` guards it); fixture `--deep` = 0.07 s; **standard-edition timing still unverified** | ◐ |
| **AC-P1-18** | Debug reader at `/debug/read/{edition}/{surah}` renders ayahs with correct RTL and a web font, is labelled "debug view", has no persistence, and is excluded from product navigation | Visual + route test | D1.12 | `cargo test -p server --test api debug` 3/3 green (re-verified 2026-09-23): RTL + "debug view" label + per-ayah markers + HTML escaping + local-only system font stack, no remote fetch. OD-04 resolved "web font" as system stack (no bundled `@font-face`; Amiri is a Phase-2 candidate only) — no font asset blocks this; the ritual walkthrough is pending | ◐ |
...
| **AC-P1-20** | ADR-0101…ADR-0114 exist, are `Accepted`, and each contains **all §48 fields** including **religious-source** and **licensing** implications; the five D1.14 docs are published | ADR lint + doc review checklist | D1.14 | 5/5 D1.14 docs published (`docs/07-technical/quran-corpus-architecture.md`, `quran-adapter-authoring.md`, `quran-citation-spec.md`; `docs/10-operations/quran-import-runbook.md`, `quran-rollback-runbook.md`); 14 ADR files exist but not all are `Accepted` | ◐ |
